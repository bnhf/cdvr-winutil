...
Describe "Get-WinUtilNodeJsVersionChoices" {
    ...
    It "returns only the latest release of each of the 3 most recent major versions" {
        ...
        $values | Should -Be @("25.1.0", "24.13.0", "22.11.0")
    ...
    It "doesn't duplicate 24.13.0 when it's already among the 3 most recent majors" {
        Mock Invoke-RestMethod {
    ...
    It "adds 24.13.0 as an extra choice when it falls outside the 3 most recent majors" {
        Mock Invoke-RestMethod {
            @(
                [pscustomobject]@{ version = "v26.0.0"; lts = $false }
                [pscustomobject]@{ version = "v25.1.0"; lts = "Lithium" }
                [pscustomobject]@{ version = "v22.11.0"; lts = "Jod" }
            )
        }
