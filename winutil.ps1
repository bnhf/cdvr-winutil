...
$sync.configs.applications = @'
{
  ...
  "WPFInstalldockerdesktop": {
    ...
    "description": "Docker Desktop is required to run several Channels DVR ecosystem add-ons, including Olivetin (EZ-Start). Opens automatically after installing, since the Docker engine doesn't actually start until you launch it at least once - also sets it to start at login, skip opening the Dashboard on startup, and integrate with the default WSL distro.",
    ...
    "postInstallCommand": "$dockerExe = Join-Path $env:ProgramFiles 'Docker\\Docker\\Docker Desktop.exe'; if (Test-Path $dockerExe) { Start-Process $dockerExe; Set-WinUtilDockerDesktopPreferences -DockerExePath $dockerExe } else { Write-WinUtilLog -Level 'WARN' -Component 'Package' -Message 'Docker Desktop executable not found at the expected location - launch it manually from the Start Menu.' }",
    "foss": false
  },
  "WPFInstallnodejs": {
    "category": "Foundational",
    "choco": "nodejs",
    "content": "Node.js",
    "description": "JavaScript runtime required by npm-distributed Channels DVR tools such as Prismcast. Asks which version to install, then downloads that version's official installer directly from nodejs.org.",
